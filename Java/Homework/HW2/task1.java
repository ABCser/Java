...
public class task1 {
    ...
    public static int[] createArray(int length, int min, int max) {
        ...
        clearFile("/Java/Homework/HW2/log.txt");
        logArray(0, array);
        return array;
    }

    public static void logArray(int index, int[] array) {
        Logger logfile = Logger.getLogger(task1.class.getName());
        String message = String.format("%d\t%s\n", index, Arrays.toString(array));
        try (FileInputStream file = new FileInputStream("Java/Homework/HW2/log.properties")) {
